...
async fn main() {
    ...
    let state = Arc::new(AppState {
        core_url: env("CORE_ENGINE_URL", "http://core-engine:8111"),
    ...
    let app = Router::new()
        ...
        .with_state(state);
    let addr = std::env::var("GATEWAY_ADDR").unwrap_or_else(|_| "0.0.0.0:8110".into());
    let listener = tokio::net::TcpListener::bind(&addr).await.unwrap();
    tracing::info!("API Gateway (ASR) on {addr}");
    axum::serve(listener, app).await.unwrap();
}

async fn health(State(s): State<Arc<AppState>>) -> Json<Health> {
    Json(Health { status: "ok".into(), version: env!("CARGO_PKG_VERSION").into(), uptime_secs: s.start_time.elapsed().as_secs() })
}
...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (h, Json(LicenseInfo {
        ...
        source_code: "https://github.com/ext-sakamoro/ALICE-ASR-SaaS".into(),
...
async fn rate_mw(
    ...
    let ok = {
        let mut e = s.rate_limiters.entry(uid).or_insert_with(|| TokenBucket::new(100000.0, 100000.0 / 3600.0));
        e.try_consume()
    };
...
async fn forward(url: &str, req: Request) -> Result<Response, (StatusCode, Json<Err>)> {
    ...
    let body = axum::body::to_bytes(req.into_body(), 64 * 1024 * 1024).await
